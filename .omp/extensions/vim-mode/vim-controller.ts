...
const VISUAL_MOTION_KEYS = new Set([
	"h",
	"j",
	"k",
	"l",
...
function previousGraphemeCol(line: string, col: number): number {
	...
	return graphemes.length === 0
		? 0
		: col - graphemes[graphemes.length - 1]!.segment.length;
}
...
function normalizeInsertCursor(
	lines: readonly string[],
	cursor: Cursor,
): Cursor {
	const lineIndex = Math.max(0, Math.min(cursor.line, lines.length - 1));
...
function cursorAtLastNormalPosition(lines: readonly string[]): Cursor {
	const lastLine = Math.max(0, lines.length - 1);
	return normalizeNormalCursor(lines, {
		line: lastLine,
		col: getLine(lines, lastLine).length,
	});
}

function nextGraphemeOffset(text: string, offset: number): number {
	const next = segmentGraphemes(
		text.slice(Math.max(0, Math.min(offset, text.length))),
	)[0];
	return next
		? Math.min(text.length, offset + next.segment.length)
		: text.length;
}
...
function classifyWordMotionGrapheme(
	segment: string,
	wordLike: boolean,
	bigWord: boolean,
): "space" | "word" | "punct" {
	...
	return wordLike && !ASCII_PUNCTUATION_PATTERN.test(segment)
		? "word"
		: "punct";
}

function segmentWordMotion(
	text: string,
	bigWord: boolean,
): WordMotionSegment[] {
	let classGroup = 0;
	...
	for (const wordSegment of wordSegmenter.segment(text)) {
		...
		for (const grapheme of segmentGraphemes(wordSegment.segment)) {
			...
			const kind = classifyWordMotionGrapheme(
				grapheme.segment,
				wordLike,
				bigWord,
			);
...
function graphemeIndexAtOrAfter(
...
function moveWordForward(
	...
	let index = graphemeIndexAtOrAfter(
		segments,
		stateOffset(state.lines, state.cursor),
	);
	for (let step = 0; step < count; step++) {
		if (index >= segments.length)
			return cursorAtLastNormalPosition(state.lines);
		const category = wordMotionClass(segments[index]!);
		if (category === "space") {
			while (
				index < segments.length &&
				wordMotionClass(segments[index]!) === "space"
			)
				index++;
		} else {
			while (
				index < segments.length &&
				wordMotionClass(segments[index]!) === category
			)
				index++;
			while (
				index < segments.length &&
	...
	return normalizeNormalCursor(
		state.lines,
		cursorFromOffset(state.lines, segments[index]!.index),
	);
}
...
function moveWordBackward(
	...
	for (let step = 0; step < count; step++) {
		...
		if (index < 0)
			return normalizeNormalCursor(state.lines, { line: 0, col: 0 });
		const category = wordMotionClass(segments[index]!);
		while (index > 0 && wordMotionClass(segments[index - 1]!) === category)
			index--;
	}
	return normalizeNormalCursor(
		state.lines,
		cursorFromOffset(state.lines, segments[Math.max(0, index)]!.index),
	);
}
...
function moveWordEnd(
	...
	for (let step = 0; step < count; step++) {
		...
		while (
			...
			wordMotionClass(segments[index + 1]!) === category
		)
			index++;
...
function matchingBracket(
	char: string,
): { open: string; close: string; forward: boolean } | undefined {
	switch (char) {
		case "(":
		...
		for (let offset = start; offset < text.length; offset++) {
			...
			if (char === match.open) depth++;
				...
				if (depth === 0)
					return normalizeNormalCursor(
						state.lines,
						cursorFromOffset(state.lines, offset),
					);
			}
		}
	} else {
...
function charMotion(
	...
	const currentCharLength =
		segmentGraphemes(line.slice(state.cursor.col))[0]?.segment.length ?? 1;
	let from =
		key === "f" || key === "t"
			? state.cursor.col + currentCharLength
			: state.cursor.col - 1;
	let found = -1;
	for (let remaining = count; remaining > 0; remaining--) {
		found =
			key === "f" || key === "t"
				? line.indexOf(char, from)
				: line.lastIndexOf(char, from);
	...
	return {
		target: normalizeNormalCursor(state.lines, {
...
function buildCharwiseRange(
	...
	if (targetOffset > fromOffset) {
		const end = motion.inclusive
			? nextGraphemeOffset(text, targetOffset)
			: targetOffset;
		return end > fromOffset ? { start: fromOffset, end } : null;
	}
	const end = motion.inclusive
		? nextGraphemeOffset(text, fromOffset)
		: fromOffset;
	return end > targetOffset ? { start: targetOffset, end } : null;
}

function editStateByOffsets(
	state: BufferState,
	start: number,
	end: number,
	replacement: string,
	...
	return {
		lines,
		cursor: normalizeInsertCursor(
			lines,
			cursorFromOffset(lines, nextCursorOffset),
		),
	};
}
...
function findQuotedSelection(
	...
	for (let index = 0; index < line.length; index++) {
		if (line[index] === quote && !isEscapedAt(line, index))
	...
	for (let index = 0; index + 1 < quotePositions.length; index += 2) {
		...
		if (!bestPair || close - open < bestPair.close - bestPair.open)
			bestPair = { open, close };
	}
	if (!bestPair) return null;
	const lineStartOffset = stateOffset(state.lines, {
		line: state.cursor.line,
		col: 0,
	});
	return {
...
function findDelimitedPair(
	...
	for (let index = 0; index < text.length; index++) {
		...
		if (char === open) stack.push(index);
			...
			if (!bestPair || openOffset > bestPair.openOffset)
				bestPair = { openOffset, closeOffset: index };
		}
	}
	return bestPair;
}

function findDelimitedSelection(
	state: BufferState,
	open: string,
	close: string,
	around: boolean,
...
function findWordSelection(
	...
	if (index >= segments.length) {
		index = Math.max(
			0,
			graphemeIndexAtOrAfter(segments, Math.max(0, cursorOffset - 1)),
		);
		while (index > 0 && wordMotionClass(segments[index]!) === "space") index--;
	...
	while (
		startIndex > 0 &&
		wordMotionClass(segments[startIndex - 1]!) === category
	)
		startIndex--;
	let endIndex = index;
	while (
		endIndex + 1 < segments.length &&
		wordMotionClass(segments[endIndex + 1]!) === category
	)
		endIndex++;
	for (let step = 1; step < count; step++) {
		...
		while (
			...
			wordMotionClass(segments[nextIndex]!) === "space"
		)
			nextIndex++;
		...
		while (
			endIndex + 1 < segments.length &&
			wordMotionClass(segments[endIndex + 1]!) === nextCategory
		)
			endIndex++;
	}
	...
	let end =
		endIndex + 1 < segments.length
			? segments[endIndex + 1]!.index
			: text.length;
	if (around) {
		let trailingIndex = endIndex + 1;
		while (
			trailingIndex < segments.length &&
			wordMotionClass(segments[trailingIndex]!) === "space"
		)
			trailingIndex++;
		if (trailingIndex > endIndex + 1) {
			end =
				trailingIndex < segments.length
					? segments[trailingIndex]!.index
					: text.length;
		} else {
			let leadingIndex = startIndex;
			while (
				leadingIndex > 0 &&
				wordMotionClass(segments[leadingIndex - 1]!) === "space"
			)
				leadingIndex--;
...
function resolveTextObjectSelection(
	state: BufferState,
	key: string,
	around: boolean,
	count: number,
...
function deleteLineRange(
	...
	return {
		lines,
		cursor: normalizeNormalCursor(lines, { line: cursorLine, col: cursorCol }),
	};
}
...
function changeLineRange(
	...
	return {
		lines,
		cursor: normalizeInsertCursor(lines, {
			line: startLine,
			col: replacementLine.length,
		}),
	};
}

function clampDeleteForwardWithinLine(
	line: string,
	startCol: number,
	count: number,
): number {
...
function clampDeleteBackwardWithinLine(
	line: string,
	startCol: number,
	count: number,
): number {
...
export class VimController {
	...
	getVisualSelection(): SelectionRange | null {
		...
		return buildCharwiseRange(state, this.visualAnchor, {
			target: state.cursor,
			inclusive: true,
		});
	}
	...
	getStatusLabel(): string {
		...
		if (this.pendingTextObject)
			pieces.push(`${this.pendingTextObject.kind === "inner" ? "i" : "a"}…`);
	...
	enterNormalModeFromInsert(): void {
		...
		this.buffer.setCursor(
			normalizeNormalCursor(state.lines, {
				line: state.cursor.line,
				col: previousGraphemeCol(line, state.cursor.col),
			}),
		);
	}
	...
	handleNormalKey(key: string): boolean {
		...
		if (this.pendingOperator) {
			...
			if (key === this.pendingOperator.key) {
				this.applyRepeatedLinewiseOperator(
					this.pendingOperator,
		...
		return this.isVisualMode()
			...
			: this.handleStandaloneNormalKey(key);
	}
	...
	private maybeAccumulateMotionCount(key: string): boolean {
		...
		if (key === "0" && this.pendingOperator.motionCountBuffer.length === 0)
			return false;
	...
	private consumeMotionCount(defaultValue = 1): number {
		...
		const count =
			this.pendingOperator.motionCountBuffer.length > 0
		...
		switch (key) {
			case "h":
				this.setNormalCursor(
					moveLeftWithinLine(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "l":
				this.setNormalCursor(
					moveRightWithinLine(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "j":
			...
			case "w":
				this.setNormalCursor(
					moveWordForward(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "W":
				this.setNormalCursor(
					moveWordForward(this.buffer.getState(), this.consumeCount(), true),
				);
				return true;
			case "b":
				this.setNormalCursor(
					moveWordBackward(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "B":
				this.setNormalCursor(
					moveWordBackward(this.buffer.getState(), this.consumeCount(), true),
				);
				return true;
			case "e":
				this.setNormalCursor(
					moveWordEnd(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "E":
				this.setNormalCursor(
					moveWordEnd(this.buffer.getState(), this.consumeCount(), true),
				);
				return true;
			case "0": {
				const state = this.buffer.getState();
			...
			case "^": {
				...
				this.setNormalCursor({
					line: state.cursor.line,
					col: firstNonBlankCol(currentLine(state)),
				});
				return true;
			}
			case "$": {
				...
				this.setNormalCursor({
					line: state.cursor.line,
					col: lastGraphemeCol(currentLine(state)),
				});
				return true;
			}
			case "%": {
			...
			case "s":
				...
				return true;
			case "S":
				this.changeWholeLines(
					this.countBuffer.length > 0 ? this.consumeCount() : 1,
				);
				return true;
			case "D":
	...
	private handlePendingG(key: string): boolean {
		...
		switch (key) {
			case "g":
				this.gotoLine(
					this.countBuffer.length > 0 ? this.consumeCount() - 1 : 0,
				);
				return true;
			case "e":
				this.setNormalCursor(
					moveWordEndBackward(this.buffer.getState(), this.consumeCount()),
				);
				return true;
			case "E":
				this.setNormalCursor(
			...
			case "_": {
				...
				this.setNormalCursor({
					line: state.cursor.line,
					col: lastNonBlankCol(currentLine(state)),
				});
				return true;
			}
			default:
	...
	private applyVisualOperator(kind: VimOperatorKind): void {
		...
		const state = this.buffer.getState();
		const start = normalizeNormalCursor(
			state.lines,
			cursorFromOffset(state.lines, selection.start),
		);
		this.visualAnchor = null;
	...
	private startOperator(
		kind: VimOperatorKind,
		key: PendingOperator["key"],
	): void {
		this.pendingOperator = {
			kind,
			key,
			count: this.consumeCount(),
			motionCountBuffer: "",
		};
	}
	...
	private resolvePendingGMotion(key: string, count: number): Motion | null {
		...
		switch (key) {
			case "g": {
				...
				return {
					target: {
						line: Math.max(0, Math.min(targetLine, state.lines.length - 1)),
						col: 0,
					},
					linewise: true,
				};
			...
			case "E":
				return {
					target: moveWordEndBackward(state, count, true),
					inclusive: true,
				};
			case "_":
				return {
	...
	private handleOperatorMotionKey(key: string): boolean {
		...
		const motion = this.resolveOperatorMotion(
			key,
			operator.count * this.consumeMotionCount(),
			operator.kind,
		);
		if (!motion) {
			if (this.pendingG) {
	...
	private resolveOperatorMotion(
		key: string,
		count: number,
		operatorKind: VimOperatorKind,
	): Motion | null {
		const state = this.buffer.getState();
		switch (key) {
			case "h":
			...
			case "w":
				if (
					operatorKind === "change" &&
					!/^\s+$/u.test(charUnderCursor(state) ?? "")
				) {
			...
			case "g":
				this.pendingG = {
					operator: {
						kind: operatorKind,
						key: operatorKind[0] as PendingOperator["key"],
						count,
						motionCountBuffer: "",
					},
				};
	...
	private applyRepeatedLinewiseOperator(
		operator: PendingOperator,
		motionCount: number,
	): void {
	...
	private applyOperatorMotion(kind: VimOperatorKind, motion: Motion): void {
		const state = this.buffer.getState();
		if (motion.linewise) {
			const { startLine, endLine } = linewiseSelection(
				state,
				motion.target.line,
			);
			...
			switch (kind) {
				...
				case "change": {
					...
					this.buffer.applyState(
						changeLineRange(state, startLine, endLine, replacementLine),
					);
					this.mode = "insert";
					return;
				}
	...
	private applyOperatorSelection(
		kind: VimOperatorKind,
		...
		if (selection.start === selection.end) {
			if (kind === "change")
				this.enterInsertMode(cursorFromOffset(state.lines, selection.start));
			return;
		}
		const nextState = editStateByOffsets(
			state,
			selection.start,
			selection.end,
			"",
		...
		if (kind === "delete") {
			nextState.cursor = normalizeNormalCursor(
				nextState.lines,
				nextState.cursor,
			);
	...
	private handlePendingFind(key: string): boolean {
		...
		this.lastFind = {
			char: key,
			direction:
				pending.key === "f" || pending.key === "t" ? "forward" : "backward",
	...
	private handlePendingReplace(key: string): boolean {
		...
		const endCol = clampDeleteForwardWithinLine(
			line,
			state.cursor.col,
			pending.count,
		);
		...
		const endOffset = stateOffset(state.lines, {
			line: state.cursor.line,
			col: endCol,
		});
	...
	private gotoLine(targetLine: number): void {
		...
		const target =
			line.length === 0
				...
				: { line: lineIndex, col: firstNonBlankCol(line) };
	...
	private moveVertical(delta: number): void {
		...
		const targetLine = Math.max(
			0,
			Math.min(state.lines.length - 1, state.cursor.line + delta),
		);
		...
		if (line.length > 0) {
			targetCol =
				goal >= line.length
					? lastGraphemeCol(line)
					: normalizeBoundary(line, goal, false);
		}
		this.preferredColumn = goal;
		this.buffer.setCursor(
			normalizeNormalCursor(state.lines, { line: targetLine, col: targetCol }),
		);
	}
	...
	private appendAfterCursor(): void {
		...
		const line = currentLine(state);
		const col =
			line.length === 0 ? 0 : stripOneNormalCharRight(line, state.cursor.col);
	...
	private openLineBelow(): void {
		...
		this.buffer.applyState({
			lines,
			cursor: { line: insertAt, col: indent.length },
		});
		this.mode = "insert";
	}
	...
	private deleteCharsUnderCursor(count: number, enterInsert: boolean): void {
		...
		this.register = {
			kind: "char",
			text: linesToText(state.lines).slice(startOffset, endOffset),
		};
		const nextState = editStateByOffsets(
			state,
			startOffset,
		...
		nextState.cursor = enterInsert
			? normalizeInsertCursor(nextState.lines, nextState.cursor)
			: normalizeNormalCursor(nextState.lines, nextState.cursor);
		this.buffer.applyState(nextState);
	...
	private deleteCharsBeforeCursor(count: number): void {
		...
		const startCol = clampDeleteBackwardWithinLine(
			line,
			state.cursor.col,
			count,
		);
		const startOffset = stateOffset(state.lines, {
			line: state.cursor.line,
			col: startCol,
		});
	...
	private deleteWholeLines(count: number): void {
		...
		const endLine = Math.min(
			state.lines.length - 1,
			state.cursor.line + count - 1,
		);
		this.register = {
			kind: "line",
			lines: state.lines.slice(state.cursor.line, endLine + 1),
		};
	...
	private changeWholeLines(count: number): void {
		...
		this.buffer.applyState(
			changeLineRange(state, state.cursor.line, endLine, replacementLine),
		);
		this.mode = "insert";
	}
	...
	private putAfter(): void {
		...
		if (this.register.kind === "line") {
			...
			const cursor = normalizeNormalCursor(lines, {
				line: insertAt,
				col: firstNonBlankCol(lines[insertAt] ?? ""),
			});
		...
		const insertCol =
			line.length === 0 ? 0 : stripOneNormalCharRight(line, state.cursor.col);
		const insertOffset = stateOffset(state.lines, {
			line: state.cursor.line,
			col: insertCol,
		});
		const nextState = editStateByOffsets(
			...
			insertOffset,
			this.register.text,
			insertOffset + this.register.text.length,
		);
		nextState.cursor = normalizeNormalCursor(nextState.lines, nextState.cursor);
		this.buffer.applyState(nextState);
	}
	...
	private joinLines(totalLines: number): void {
		...
		const cursor = normalizeNormalCursor(lines, {
			...
			col: Math.min(originalCursor.col, joined.length),
		});
		this.buffer.applyState({ lines, cursor });
		this.mode = "normal";
	}
