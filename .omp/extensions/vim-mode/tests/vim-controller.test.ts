...
function parseMarkedText(marked: string): {
	lines: string[];
	...
	return {
		lines,
		cursor: { line: lines.length - 1, col: lines.at(-1)?.length ?? 0 },
	};
}
	...
	return state.lines
		.map((line, index) =>
			index === state.cursor.line
				? `${line.slice(0, state.cursor.col)}|${line.slice(state.cursor.col)}`
				: line,
		)
		.join("\n");
}
...
function feed(controller: VimController, ...keys: string[]): void {
	for (const key of keys) {
		assert.equal(
			controller.handleNormalKey(key),
			true,
