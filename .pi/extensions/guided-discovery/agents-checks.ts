...
function isLikelyShellCommand(value: string): boolean {
	...
	return /[\s/]|^(npm|pnpm|yarn|bun|node|deno|python|pytest|cargo|go|make|just|jj|git|uv|poetry|pip|npx|tsx|vitest|jest|mocha|ruff|eslint|prettier|biome|turbo|nx|gradle|mvn|dotnet|swift|xcodebuild|dart|flutter|phpunit|composer|bundle|rspec|mix|zig|cmake|ctest)\b/i.test(
		command,
	);
}
...
export function extractAgentsCheckCommands(markdown: string, source: string): AgentsCheckCommand[] {
	...
	for (const rawLine of lines) {
		...
		if (explicit) {
			...
			if (isLikelyShellCommand(command)) commands.push({ command, source });
			continue;
		}
		...
		if (!inRelevantFence || !inRelevantSection) continue;
...
export function buildAgentsCheckFindings(runs: AgentsCheckRun[]): CheckerFinding[] {
	return runs
		...
		.map((run, index) => ({
			...
			summary: `Required AGENTS.md check failed: ${run.command}`,
			details: `${run.source}: ${run.summary}`,
			suggestedFix: `Make the implementation pass \`${run.command}\` as requested by ${run.source}.`,
			paths: [],
